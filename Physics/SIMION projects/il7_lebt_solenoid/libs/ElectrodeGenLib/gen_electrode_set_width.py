...
#       D4, D5, D6, DX: A set of three discs that have a central
#                   hole of 4, 5, 6 or Xmm. They can be conbined
#                   as we want. The width of the disk is wmm.
#   The minimum distance between electrodes is:
#       a = 5mm, b = 3mm, c = 3mm.
#   The electrodes are rounded at their lower edges with a radius
#   of 2.2mm.
...
#-- define the grid granularity 0.1mm/gu. Since it is cylindrical
#-- we only define dx_mm and dy_mm
dx_mm = 0.1
dy_mm = dx_mm

#-- 0.1mm/gu => 10 gu/mm. We need to define it to avoid floats
#-- in conversions
M2G = int(1/dx_mm)

#-- Our grid needs to be at least PE+a+Di+b+Di = 4+5+3+3+3 = 18mm
#-- in the x direction and 45mm in the y one.
#-- if our granularity will be 0.1mm/gu we will need at least 450g
#-- nx = 72 and ny = 170. Since we want the simulation from PE to our beam
#-- dump we will expand the grid until nx = 44.6cm = 446 mm = 4460 gu
NX0 = 10061
NY0 = 1601
...
# PD radious
PDr = 2               # Radius of the electrode ending rounding in mm

# TUBE

TX0 = 228.8 # Tube starting point
TX1 = 902.8 # Tube end Nominal 832.3
TY0 = 19  # Tube internal radius
TY1 = 20.5  # tube external radius
...
def gen_electrode_abs_pos (pa, geom, width, posx, elec_num):
    ''' # Name: gen_electrode_abs_pos (geom, width, posx, elec_num)
        ...
        #   of the selected electrode rounding its edges, in the given
        #   position with the stated reference number.'''
    ...
    if geom == 'PE':
        ...
        Dr = float(geom[1:])/2       # Extract the radius of the  # electrode from the name.                              
        for x in range(int(posx*M2G), int(posx*M2G + width*M2G)+1):
            for y in range(int(Dr*M2G), int(Pr*M2G)):
                isinside = True
                # Check if the point x,y is in the lower left part of the electrode.
                if ((y <= int (M2G*(Dr+PDr)) ) and ( x <= int(M2G*(posx+PDr))) ):
                    # Check if it is part of the rounding or not.
                    if ((x - int(M2G*(posx+PDr)))**2 + (y - int (M2G*(Dr+PDr)))**2) > (M2G*PDr)**2 :
                        isinside = False
                # Check if the point x,y is in the lower right part of the electrode.
                if ((y <= int (M2G*(Dr+PDr)) ) and  (x >= int(M2G*(posx+width-PDr))) ) :
                    # Check if it is part of the rounding or not.
                    if ((x - int(M2G*(posx+width-PDr)))**2 + (y - int (M2G*(Dr+PDr)))**2) > (M2G*PDr)**2 :
                        isinside = False
                if Dr == 0: # If the hole diameter is 0 it is because we do not want any hole.
                    isinside = True
                if isinside:
                    pa.point(x, y, z, 1, elec_num)
    return pa

def gen_set_electrodes (pa, x_width, width0,
                        dist1, elec1, width1,
                        dist2, elec2, width2,
                        dist3, elec3, width3):
    ''' # Name: gen_set_electrodes (pa, dist1, elec1,
        ...
        # Inputs:
        #   pa: pa template to modify
        #   x_width: Distance in x axis from nominal when solenoids are
        #            enlarged.
    ...
    #pa = gen_electrode_abs_pos(pa, 'D0', 1, (NX/M2G)-1.1, 5)
    # The position is NX0 + 2* each solenoid enlargement - width of the elecrode.
    pa = gen_electrode_abs_pos(pa, 'D0', 1, ((NX0 - 2)/M2G) + 2*x_width, 5)
    #pa = gen_electrode_abs_pos(pa, 'D0', 1, 300, 5)

    return pa
    
def gen_tube (pa,x_width,x0,y0,x1,y1): 
    ''' Taking into account a cylindrical symemtry this function generates
        a tube from a box, specifying inital and end coordinates.
        x_width: Distance in x axis from nominal when solenoids are enlarged.
        x0: initial coordinate in mm. x-axis
        x1: end coordinate in mm. x-axis
        y0: initial coordinate in mm. y-axis
        y1: end coordinate in mm. y-axis      

    '''  
    z = 0 # We don't need this parameter because our symmetry is cylindrical
    for x in range(int(x0*M2G), int((x1 + 2*x_width)*M2G)):
        for y in range(int(y0*M2G), int(y1*M2G)):
            pa.point(x, y, z, 1, 6)
          
    return pa          
    
def gen_electrodes_set_pa (x_width, y_height, dists, elecs, width, filename):
    #-- creating an array from scratch
    #-- NY and NY are defiend in ElecrtrodeGenLib.gen_electrode_set_width
    # nx is NX0 + 2* each solenoid enlargement.
    pa = PA(nx = NX0 + int(2*x_width*M2G), ny = NY0 + int(y_height*M2G), symmetry = 'cylindrical')

    # Generate a set of electrodes
    pa = gen_set_electrodes (pa, x_width, width[0],
                        dists[0], elecs[0], width[1],
                        dists[1], elecs[1], width[2],
                        dists[2], elecs[2], width[3])

    pa = gen_tube (pa, x_width, TX0, TY0, TX1, TY1)                          
