...
def gen_set_electrodes (pa, width0,
    ...
    pa = gen_electrode_abs_pos(pa, 'D0', 1, ((NX-2)/M2G), 5)
    #pa = gen_electrode_abs_pos(pa, 'D0', 1, 300, 5)

    return pa
    
def gen_tube (pa,x0,y0,x1,y1): 
    ''' Taking into account a cylindrical symemtry this function generates a tube from
        a box, specifying inital and end coordinates
    ...
    '''  
    z = 0 # We don't need this parameter because our symmetry is cylindrical
    for x in range(int(x0*M2G), int(x1*M2G)):
