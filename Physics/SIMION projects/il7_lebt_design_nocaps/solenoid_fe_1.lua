...
-- USER defined params
  ...
  -- For SIMULATION
local DONOTPRESERVETRAJ = 0 --If true trajectories are NOT preserved.
...
local ELECTRVSTEP = 500   -- 3rd electrode voltage increment step
  ...
  -- Plasma electrode
local plasmaelecvolt = 20000 -- plasma electrode voltage
...
local MIN_X = 0.01
  ...
  -- LEBT
adjustable SOL1I0 = 18500 -- A. Solenoid 1 initial total current.
adjustable SOL1IF = 18500 -- A. Solenoid 1 final total current.
...
function segment.flym()
  ...
  for _, datum in ipairs(data) do
    plasmaelecvolt = datum.plasma_volt -- plasma electrode voltage
    ELECTRV0 = datum.elec_init_volt     -- 3rd electrode initial voltage
    ELECTRVF = datum.elec_end_volt     -- 3rd electrode final voltage
    ELECTRVSTEP = datum.elec_step_volt   -- 3rd electrode voltage step
    SOL1I0 = datum.sol1_init_total_A --Solenoid 1 initial total current in A
    SOL1IF = datum.sol1_end_total_A --Solenoid 1 final total current in A
    SOL1ISTEP = datum.sol1_step_total_A --Solenoid 1 step total current in A
    SOL2I0 = datum.sol2_init_total_A --Solenoid 2 initial total current in A
    SOL2IF = datum.sol2_end_total_A --Solenoid 2 final total current in A
    SOL2ISTEP = datum.sol2_step_total_A --Solenoid 2 step total current in A
    ...
    for I1T = SOL1I0, SOL1IF, SOL1ISTEP do
      ...
      for I2T = SOL2I0, SOL2IF, SOL2ISTEP do
        ...
        print('B-field (Gauss) measured at (416,0,0) mm:', bfield(416,0,0))
        print('B-field (Gauss) measured at (416,5,0) mm:', bfield(416,5,0))
        print('B-field (Gauss) measured at (416,15,0) mm:', bfield(416,15,0))
        print('B-field (Gauss) measured at (730,0,0) mm:', bfield(730,0,0))
        print('B-field (Gauss) measured at (730,5,0) mm:', bfield(730,5,0))
        print('B-field (Gauss) measured at (730,15,0) mm:', bfield(730,15,0))
        print('B-field (Gauss) measured at (730,20,0) mm:', bfield(730,20,0))
        print('B-field (Gauss) measured at (730,25,0) mm:', bfield(730,25,0))
        print('B-field (Gauss) measured at (730,35,0) mm:', bfield(730,35,0))
